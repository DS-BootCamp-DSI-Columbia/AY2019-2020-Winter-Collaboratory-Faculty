...
1. See how professional poker players update their beliefs about their opponents' face-down
  cards as they accumulate more data in the form of bets
...
two face-down cards. For more details on the rules, you can refer to

https://en.wikipedia.org/wiki/Texas_hold_%27em

At the outset of this video, the five players have the following amounts of poker chips:
```{r}
stacks <- c(Salomon  = 27450000, # must post the "big blind"
            Kaverman =  8025000, # has to start the betting
            Holz     = 28675000,
            Bonomo   = 51150000,
            Smith    = 19700000) # must post the "small blind"
```
...
standard for a poker player in this situation, namely one who has much fewer poker
chips than all the other players at the table. Specifically, simulations have shown
...
* Big pairs (Js or better), of which there are 4 pairs $\times$ 6 suit combinations for 24 hands:
...
* Medium pairs (6s or better), of which there are 4 pairs $\times$ 6 suit combinations for 24 hands:
...
* Ace and at least a nine of the same suit, of which there are 5 intersections $\times$ 4 suits for 20 hands:
...
* Both cards at least 10 of the same suit, of which there are 6 intersections $\times$ 4 suits for 24 hands:
...
* Ace and at least a Jack of different suit, of which there are 6 intersections $\times {4 \choose 2}$ suits for 36 hands:
...
has `r card("A", "C")` `r card("5", "C")`? Can you write this posterior probability in the form of Bayes' Rule?**
...
**If Kaverman has a pair of Jacks, Queens, Kings, or Aces, what is the probability that Holz's five-card hand ends up
...
# Rick Saloman's Decision

After Holz calls Kaverman's bet and both Justin Bonomo and Dan Smith fold, $17,550,000$ chips have been bet. Rick
Saloman is dealt `r card("A", "H")` `r card("K", "H")` and bets all of his remaining $26,730,000$ chips. One of
the commentators says about 1:38 into the video that Saloman would not do so unless he had a pair of Jacks,
...
Between 1:03 and 1:04 of the video, Saloman picks up the `r card("A", "H")` in his right hand in such a way
...
At this point, Holz knows that Saloman's remaining face-down card is one of `r card("A", "S")`, `r card("A", "D")`,
`r card("A", "C")`, `r card("K", "S")`, `r card("K", "H")`, `r card("K", "D")`, or `r card("K", "C")`.

## Beating an Offsuit King

Suppose Saloman's remaining face-down card is a King but not `r card("K", "H")`. **What is the probability that 
Holz's five-card hand ends up being better than Saloman's five-card hand?** For the purposes of this question, 
you can ignore the tiny probability that either player ends up with a straight flush, four of a kind, full house, 
flush, or straight.

## Probability that Saloman has another Ace
...
Ace but that influence's [Holtz's] decision". **From Holtz's perspective, what is the probability that
Saloman's remaining face-down card is an Ace?**
...
and Kaverman would have lost when Salomon made two pair. The four remaining amounts of poker chips would be
```{r}
...
place is equal to the proportion of chips each has *excluding the chips of the player who wins
...
similar. **Under the above assumptions, what would be the expected amount of prize money that
Salomon would win, as compared to the $2.84 million he actually won?**
